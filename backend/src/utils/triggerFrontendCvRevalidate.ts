...
const resolveEndpoints = (): string[] => {
  ...
  if (frontendUrls.length > 0) {
    return frontendUrls.map((frontendUrl) => `${frontendUrl}/api/revalidate/cv`)
  }

  const publicUrl = (process.env.PUBLIC_SERVER_URL || '').trim()
  if (!publicUrl) return []

  return [`${withNoTrailingSlash(publicUrl)}/api/revalidate/cv`]
}
...
type TriggerFrontendCvRevalidateOptions = {
  warmPaths?: string[]
}

export const triggerFrontendCvRevalidate = async (
  reason: string,
  options: TriggerFrontendCvRevalidateOptions = {},
): Promise<void> => {
  ...
  const secret =
    (process.env.FRONTEND_CV_REVALIDATE_SECRET || '').trim() ||
  ...
  if (!secret && !isLocal) {
    if (!isLocalProfile() && !hasWarnedMissingSecret) {
      ...
      console.warn(
        '[revalidate] FRONTEND_CV_REVALIDATE_SECRET and FRONTEND_PROJECTS_REVALIDATE_SECRET are not set; skipping CV revalidation ping.',
      )
    }
    return
  }
  ...
  if (failures.length > 0) {
    console.warn(
      `[revalidate] Frontend CV revalidation failed for all configured endpoints: ${failures.join('; ')}`,
    )
  }
}
