...
if [[ "${ALLOW_DEV_WRITE:-}" != "true" ]]; then
  ...
  echo "Example: ALLOW_DEV_WRITE=true ./scripts/ops/import-project-descriptions-dev.sh"
  exit 1
fi

read -r -p "Type 'import-projects-dev' to confirm dev write: " confirm
if [[ "${confirm}" != "import-projects-dev" ]]; then
