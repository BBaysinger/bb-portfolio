...
You discovered that the local private key `~/.ssh/bb-portfolio-site-key.pem` does **not** match the key pair actually embedded in the EC2 instance(s). To avoid rebuilding the candidate instance, you can temporarily grant _scoped_ SSM permissions, inject your current public key, and then revoke the permission.
