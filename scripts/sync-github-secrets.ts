...
try {
  ...
  console.error(
    "Error: failed to fetch current GitHub secrets. Is gh authenticated?",
  );
  process.exit(1);
}

const desiredKeys = [...Object.keys(data.strings), ...Object.keys(data.files)];
...
for (const [key, value] of Object.entries(data.strings)) {
  if (DRY_RUN) {
    console.log(
      `🔑 (dry run) Would set string ${key} (length: ${value.length})`,
    );
  } else {
...
for (const [key, filepath] of Object.entries(data.files)) {
  ...
  if (DRY_RUN) {
    ...
    console.log(
      `📄 (dry run) Would set file ${key} from ${filepath} (${size} bytes)`,
    );
  } else {
...
if (DRY_RUN) {
  ...
  console.log(
    `✅ Sync complete! Repo ${REPO} now matches ${JSON_FILE} exactly.`,
  );
}
