...
Examples:
  ./scripts/dockerhub-ensure-public.sh
  ./scripts/dockerhub-ensure-public.sh --repositories ns/app1,ns/app2
USAGE
}
...
echo "🔧 Ensuring Docker Hub repos are public: ${REPOS[*]}"
...
for repo in "${REPOS[@]}"; do
  ...
  if [[ "$code" == "404" ]]; then
    # Create repository as public (is_private=false)
    ns="${repo%%/*}"; name="${repo##*/}"
    echo "  ℹ️  Repo not found. Creating $ns/$name as public..."
    create_code=$(curl -sS -o /dev/null -w "%{http_code}" -X POST "${AUTH_HEADER[@]}" -H 'Content-Type: application/json' \
      -d '{"name":"'"$name"'","namespace":"'"$ns"'","is_private":false}' "https://hub.docker.com/v2/repositories/")
    if [[ "$create_code" =~ ^20[01]$ ]]; then
      echo "  ✅ Created public repository"
      rm -f "$body"
      continue
    else
    ...
    patch_code=$(curl -sS -o /dev/null -w "%{http_code}" -X PATCH "${AUTH_HEADER[@]}" -H 'Content-Type: application/json' \
      -d '{"is_private": false}' "https://hub.docker.com/v2/repositories/${repo}/")
    if [[ "$patch_code" =~ ^20[04]$ ]]; then
      echo "  ✅ Set to public"
    else
      echo "  ❌ Failed to set public (HTTP $patch_code)." >&2
    fi
    rm -f "$body"
  else
