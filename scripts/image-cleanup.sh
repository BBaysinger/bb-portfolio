...
#   ./scripts/image-cleanup.sh --retain 10 --repositories bb-portfolio-backend-prod,bb-portfolio-frontend-prod --region us-west-2
#   ./scripts/image-cleanup.sh --retain 5 --include-untagged --dry-run
#   ./scripts/image-cleanup.sh --profile my-aws --region us-west-2
set -euo pipefail
...
for repo in "${REPOS[@]}"; do
  ...
  mapfile -t tagged < <(aws ecr describe-images \
  ...
  if $INCLUDE_UNTAGGED; then
    mapfile -t untagged < <(aws ecr list-images \
      --repository-name "$repo" \
