...
# Usage:
#   ./scripts/migrate-ec2-directory.sh --host ec2-user@<IP_OR_DNS> --key ~/.ssh/bb-portfolio-site-key.pem [--dry-run]
#
...
# 4. Adjust ownership & permissions.
...
usage() {
  cat <<EOF
Usage: $0 --host ec2-user@IP --key /path/to/key.pem [--dry-run]
...
if [[ ! -d "$OLD" && -d "$NEW" ]]; then
  ...
  if [[ -d "$NEW" && -d "$OLD" ]]; then
    ...
    mv "$OLD" "/home/ec2-user/portfolio_backup_$(date +%Y%m%d_%H%M%S)";
  elif [[ -d "$OLD" ]]; then
...
log "Preparing migration (dry-run=$DRY_RUN)"
