...
const FRONTEND: { host: string; port: number; paths: string[] } = { host: 'localhost', port: 5050, paths: ['/'] };
...
const RETRY_INTERVAL = parseInt(process.env.WARM_RETRY_INTERVAL_MS || '2000', 10);
...
async function waitForReachable(host: string, port: number, label: string, path: string) {
  const start = Date.now();
  console.log(`Waiting for ${label} on http://${host}:${port}${path} ...`);
  while (true) {
    const code = await httpGet(host, port, path);
    if (code > 0) {
      console.log(`✓ ${label} reachable on http://${host}:${port}${path} (status ${code})`);
      return;
    }
    if (Date.now() - start > TIMEOUT) {
      throw new Error(`Timeout waiting for ${label} on http://${host}:${port}${path}`);
    }
...
async function warm(host: string, port: number, paths: string[], label: string) {
