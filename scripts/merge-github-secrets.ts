...
export const mergeGithubSecrets = (options: MergeOptions = {}): MergeResult => {
  ...
  for (const input of inputs) {
    ...
    if (!quiet) {
      console.info(
        `🔁 merged ${input.label} secrets (${path.relative(rootDir, absolute)})`,
      );
    }
  }
  ...
  if (!quiet) {
    console.info(
      `✅ Wrote bundled secrets → ${path.relative(rootDir, outputPath)}`,
    );
    if (missing.length) {
      const skipped = missing
        .map((m) => `${m.label} (${m.relativePath})`)
        .join(", ");
      console.info(`ℹ️  Skipped missing inputs: ${skipped}`);
    }
  }
