...
resource "null_resource" "iac_validation" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      ssh -i ~/.ssh/bb-portfolio-site-key.pem -o ConnectTimeout=10 -o StrictHostKeyChecking=no ec2-user@${aws_eip.portfolio_ip.public_ip} '
      ...
      if curl -s -I http://${aws_eip.portfolio_ip.public_ip} | head -1 | grep -q "200 OK"; then
        echo "✅ Site accessible externally at http://${aws_eip.portfolio_ip.public_ip}"
      else
  ...
  depends_on = [aws_eip_association.portfolio_assoc]
}