...
watch_timeout_secs=120  # max seconds to wait when watching GH workflow status
...
Options:
  ...
  --watch-timeout [secs]  Max seconds to wait for GH run to finish when watching (default: 120)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-watch) watch_logs=false; shift ;;
      --watch-timeout)
        watch_timeout_secs="${2:-}"; [[ "$watch_timeout_secs" =~ ^[0-9]+$ ]] || die "--watch-timeout requires an integer seconds value"; shift 2 ;;
      ...
      if [[ $STATUS -eq 0 ]]; then
        ...
        if [[ -n "$RUN_ID" ]]; then
          ...
          if [[ "$watch_logs" == true ]]; then
            log "Watching run status up to ${watch_timeout_secs}s ($ENV_IN)"
            elapsed=0
            status=""; conclusion=""
            while (( elapsed < watch_timeout_secs )); do
              info=$(gh run view "$RUN_ID" --repo "$GH_REPO" --json status,conclusion,url 2>/dev/null || true)
              status=$(jq -r '.status // empty' <<<"$info" 2>/dev/null || true)
              conclusion=$(jq -r '.conclusion // empty' <<<"$info" 2>/dev/null || true)
              [[ -z "$RUN_URL" ]] && RUN_URL=$(jq -r '.url // empty' <<<"$info" 2>/dev/null || true)
            ...
            if [[ "$status" != "completed" && -z "$conclusion" ]]; then
              warn "Run $RUN_ID watch timed out after ${watch_timeout_secs}s. ${RUN_URL:+See: $RUN_URL}"
            else
              ok "Run $RUN_ID finished with status=$status conclusion=${conclusion:-unknown}"
...
# Quick public HTTP health check against resolved host (Elastic IP)
public_health_check() {
  local host="$1"
  [[ -n "$host" ]] || return 0
  log "Public HTTP health check (http://$host)"
  local i=0
  while (( i < 6 )); do
    local head
    head=$(curl -sI --max-time 5 "http://$host" | head -1 || true)
    if [[ -n "$head" ]]; then
      echo "$head"
      if echo "$head" | grep -Eq "HTTP/.* (200|301|302)"; then
        ok "Public HTTP responding"
        return 0
      fi
    fi
    sleep 5; i=$((i+1))
  done
  warn "No HTTP response after ~30s"
  return 0
}
...
case "$profiles" in
  prod)
    ...
    if dispatch_redeploy prod main "$BRANCH"; then
      host=$(resolve_ec2_host || true)
      [[ -n "$host" ]] && public_health_check "$host"
